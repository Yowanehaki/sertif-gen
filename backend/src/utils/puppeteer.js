...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .certificate-content { position: absolute; width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; }
    .participant-name { position: absolute; top: 935px; left: 50%; transform: translateX(-50%); font-size: 141px; font-weight: 700; color: black; text-align: center; font-family: 'Montserrat', sans-serif; }
    .activity { position: absolute; top: 1365px; left: 50%; transform: translateX(-50%); font-size: 75px; font-weight: 600; color: black; text-align: center; font-family: 'Montserrat', sans-serif; white-space: nowrap; max-width: 90%; overflow: hidden; text-overflow: ellipsis; }
    .date { position: absolute; top: 1460px; left: 50%; transform: translateX(-50%); font-size: 46px; font-weight: 400; color: black; text-align: center; font-family: 'Montserrat', sans-serif; }
    .signature { position: absolute; top: 1580px; left: 50%; transform: translateX(-50%); width: 550px; height: 250px; }
    .examiner-name { position: absolute; top: 1931px; left: 50%; transform: translateX(-50%); font-size: 87px; font-weight: 600; color: black; text-align: center; font-family: 'Montserrat', sans-serif; }
    .examiner-position { position: absolute; top: 2044px; left: 50%; transform: translateX(-50%); font-size: 79px; font-weight: 400; color: black; text-align: center; text-decoration: underline; font-family: 'Montserrat', sans-serif; }
    .company-code { position: absolute; top: 2133px; left: 95px; font-size: 42px; font-weight: 700; color: black; font-family: 'Montserrat', sans-serif; }
    .validation-text { position: absolute; top: 2191px; left: 95px; font-size: 41px; font-weight: 400; color: black; font-family: 'Montserrat', sans-serif; }
    .validation-id{ font-weight: 410; }
  </style>
</head>
<body>
  <div class="certificate-content">
    ...
    ${data.signaturePath && fs.existsSync(data.signaturePath) ? `<img class="signature" src="data:image/png;base64,${fs.readFileSync(data.signaturePath, { encoding: 'base64' })}" alt="Signature">` : ''}
    ...
    <div class="validation-text">This certificate can be validated ( ID : <span class="validation-id">${this.escapeHtml(data.serialNumber)}</span> )</div>
      ...
      return { success: true, filename, outputPath, fontsUsed: 'Montserrat (Google Fonts)' };
    } catch (error) {
  ...
  async generateCertificatePDF(data, pdfPath) {
    ...
    try {
      ...
      const formattedDate = new Date(data.dateIssued).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      const templatePath = path.join(__dirname, '../../assets/certificate-template.png');
      if (!fs.existsSync(templatePath)) throw new Error('Certificate template not found');
      const templateBase64 = fs.readFileSync(templatePath, { encoding: 'base64' });
      let signatureImg = '';
      ...
      await page.pdf({ path: pdfPath, printBackground: true, width: `${width}px`, height: `${height}px` });
